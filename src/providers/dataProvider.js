...
const dataProvider = {
    ...
    if (resource === 'staff') {
      ...
      try {
        ...
        if (!json.staff || !Array.isArray(json.staff)) {
          console.error("Expected an array but received:", json.staff);
          return { data: [], total: 0 };
        }
    
        return {
          data: json.staff.map((staff) => ({ id: staff._id, ...staff })),
          total: json.staff.length,
        };
  ...
  create: async (resource, params) => {
    ...
    if (resource === 'staff') {
      ...
      if (!data.get('en_name') || !data.get('en_position') || !data.get('en_description')) {
        throw new Error("Required doctor data (name, position, phoneNumber) is missing");
      }
      ...
      return { data: { ...data, id: createdStaff.id } };
    }
    ...
    if (resource === 'staff' && id) {
      ...
      try {
        ...
        if (response.status === '200') {
          ...
          return Promise.reject(new Error('Failed to delete Staff'))
        }
      } catch (error){
  ...
  update: async (resource, params) => {
    ...
    if (resource === 'staff') {
      const url = `${apiUrl}/staff/${id}/update`;
      const { en_name, 
        uz_name, 
        ru_name, 
        en_position, 
      ...
      try {
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          return Promise.reject(new Error("Failed to update Staff: unexpected status."));
        }
      } catch (error) {
        console.error("Update error:", error);
        return Promise.reject(new Error("Failed to update resource."));
